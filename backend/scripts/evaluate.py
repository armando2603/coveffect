...
class Evaluator:
    ...
    def compute_corpus_scores(self, predictions_targets_list):
        ...
        predictions_targets_list['target_values'] = predictions_targets_list.apply( lambda row: list(set(row['target_values'])), axis=1)
        predictions_targets_list['prediction_values'] = predictions_targets_list.apply( lambda row: list(set(row['prediction_values'])), axis=1)
        ...
        len_by_doi = general_scores[['doi', 'targets']]
        len_by_doi['targets_len'] = len_by_doi.apply( lambda row: len(row['targets']), axis=1) 
        len_by_doi = len_by_doi[['doi', 'targets_len']].groupby(['doi']).sum()
