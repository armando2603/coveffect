...
class Predictor:
    ...
    def prediction(self, conditional, pre_input_ids):
        conditional_ids = self.tokenizer.encode(
            conditional,
            return_tensors='pt',
            truncation=True,
        ...
        return generated_sequence, distributions
    ...
    def generateTable(self, inputs, fields):
        ...
        with torch.no_grad():
            for it, input_text in enumerate(tqdm(inputs)):
                ...
                nested_outputs = []
                conditionals = []
                for field in self.fields:  # L'ordine dei fields o attribute conta
                    for cond_index, conditional in enumerate(conditionals):
                        if field['multiple_output']:
                            conditional += field + '_multiple: '
                        else:
                            conditional += field + ': '
                        
                        generated_sequence, distributions = self.prediction (conditional, pre_input_ids)
                        
                        # TODO output è una lista e da ogni lista parte un record
                        # devo decidere un conditional input per i valori multipli
                        values, confidences = self.generated_sequence_parser(
                            generated_sequence, distributions, field
                        )
                        for index, value, confidence in enumerate(zip(values, confidences)):
                                attributes_dicts.append(dict(
                                    value=value.strip(),
                        ...
                        for value in values:
                            conditionals.append(field + ': ' + value.strip + '<SEPO>')
